...
This MCP server is built FOR Claude (you). When using these tools:

### Mindset

You are not just calling tools - you're the primary user validating they work correctly.

### When Tools Fail

| ❌ Don't | ✅ Do |
|----------|-------|
| Silently pivot to workarounds | Stop and diagnose why it failed |
| Guess missing parameters | Flag incomplete `getToolInfo()` as a bug |
| Assume you made a mistake | Consider the tool might have a bug |

### Quality Check

Before working around a tool failure, ask:

1. **Did `getToolInfo()` show all required params?** If not → definition bug
2. **Was the error message helpful?** If not → error handling bug  
3. **Did the tool do what the description said?** If not → implementation bug

### Dogfooding Principle

If you can't use a tool confidently with ONLY the information from `listAvailableTools()` and `getToolInfo()`, that's not your failure - it's a gap in the tool's discoverability that needs fixing.

### Report Issues

When something doesn't work as expected, tell the human:
...
This helps improve the server for all future Claude instances.
