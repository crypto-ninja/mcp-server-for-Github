# Cursor Prompt: Phase 1 - Create compact_format.py

## Task
Create `src/github_mcp/utils/compact_format.py` with serializers for compact output format.
...
def compact_commit(commit: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "author": commit.get("commit", {}).get("author", {}).get("name") or commit.get("author", {}).get("login", "unknown"),
...
def compact_alert(alert: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "severity": alert.get("security_advisory", {}).get("severity") or alert.get("rule", {}).get("severity", "unknown"),
...
Run quick test:
```bash
python -c "from github_mcp.utils.compact_format import to_compact; print('OK')"
```

Then tell me "Phase 1 complete" and I'll give you Phase 2.
