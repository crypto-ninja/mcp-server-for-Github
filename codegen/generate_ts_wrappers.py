...
class MCPToolExtractor:
    ...
    def _extract_tool_info(
        self, node: ast.AsyncFunctionDef, decorator
    ) -> Optional[ToolInfo]:
                ...
                if keyword.arg == "name":
                    ...
                    if isinstance(keyword.value, ast.Dict):
                        for k, v in zip(keyword.value.keys, keyword.value.values):
                            ...
                            elif isinstance(k, ast.Str) and isinstance(
                                v, ast.Str
                            ):  # Python < 3.8
                                annotations[k.s] = v.s
...
class TypeScriptGenerator:
    ...
    def generate_wrapper(self, tool: ToolInfo) -> str:
        ...
        interface_name = (
            f"{self._to_pascal_case(tool.function_name)}Input" if tool.fields else "any"
        )
...
"""
    ...
    def generate_category_index(self, category: str, tools: List[ToolInfo]) -> str:
        ...
        exports = [
            f"export * from './{self._to_kebab_case(tool.function_name)}.js';"
            for tool in tools
        ]
    ...
    def generate_main_index(self, categories: Dict[str, List[ToolInfo]]) -> str:
        ...
        exports = [
            f"export * as {category} from './{category}/index.js';"
    ...
    def generate_all(self, output_dir: Path):
        ...
        for category, tools in categories.items():
            ...
            for tool in tools:
                file_path = (
                    category_dir / f"{self._to_kebab_case(tool.function_name)}.ts"
                )
...
def main():
    ...
    parser.add_argument(
        "--output-dir", default="servers", help="Output directory for generated files"
    )
    ...
    try:
        ...
        if len(tools) == 0:
            print(
                "[WARNING] No tools found! Check if @mcp.tool() decorators are present."
            )
            return 1
