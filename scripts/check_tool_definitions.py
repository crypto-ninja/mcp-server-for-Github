...
def parse_python_model(model_name: str) -> Optional[Dict[str, Dict]]:
    ...
    for field_match in re.finditer(field_pattern, class_body):
        ...
        is_required = "default=" not in field_match.group(
            0
        ) and "default=None" not in field_match.group(0)

        fields[field_name] = {"required": is_required, "type": field_type}
...
def parse_typescript_tool_definitions() -> Dict[str, Dict]:
    ...
    for block in tool_blocks:
        ...
        if param_section:
            ...
            # Find each parameter
            param_pattern = (
                r'(\w+):\s*\{\s*type:\s*"([^"]+)",\s*required:\s*(true|false)'
            )
            for param_match in re.finditer(param_pattern, param_content):
                param_name = param_match.group(1)
                param_type = param_match.group(2)
                param_required = param_match.group(3) == "true"
                params[param_name] = {"type": param_type, "required": param_required}

        tools[tool_name] = {"parameters": params}
