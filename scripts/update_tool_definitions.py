                        ...
                        if item.value:
                            ...
                            if isinstance(item.value, ast.Call):
                                if len(item.value.args) > 0:
                                    if (
                                        isinstance(item.value.args[0], ast.Constant)
                                        and item.value.args[0].value == Ellipsis
                                    ):
                                        is_required = True
...
def update_tool_definition(
    ts_content: str, tool_name: str, fields: Dict[str, Dict]
) -> str:
    ...
    for field_name, field_info in sorted(fields.items()):
        ...
        param_lines.append(
            f'        {field_name}: {{ type: "{param_type}", required: {str(required).lower()}, description: "{field_name}" }},'
        )
