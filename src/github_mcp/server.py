...
def register_all_tools() -> None:
    ...
    for tool_name in all_tools:
        ...
        is_readonly = metadata.get("readOnlyHint", 
            tool_name.startswith("github_list") or 
            tool_name.startswith("github_get") or 
            tool_name.startswith("github_search") or
            tool_name.startswith("github_check"))
        is_destructive = metadata.get("destructiveHint",
            "delete" in tool_name or 
            "remove" in tool_name or
            "close" in tool_name)
        is_idempotent = metadata.get("idempotentHint",
            not ("create" in tool_name or "update" in tool_name or "delete" in tool_name))
